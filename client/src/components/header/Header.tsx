...
const Header = () => {
    ...
    const handelSersh = async (e: any) => {
        if (e.key === "Enter" || e === "Clicked") {
            if (sersh) {
                setLoadingBtn(true)
                history(`/?serch=${sersh.split(' ').join('')}`)
                await sesrshQurey(sersh.split(' ').join('')).then((res) => {
                    dispatch({ type: actionTypes.GET_SEARCH, payload: res.data.data })
                    setLoadingBtn(false)
                })
                setSersh("")
                setLoadingBtn(false)
                            ...
                            <div className="  flex  ">
                                <div className=" relative flex items-center w-full">
                                    ...
                                    {LoadingBtn ? (
                                        ...
                                        <button onClick={() => handelSersh("Clicked")} className=" px-3 ml-2 hover:rounded-2xl rounded py-2.5 bg-blue-600 text-white font-medium text-xs leading-tight uppercase shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700  focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out flex items-center" type="button" id="button-addon2">
